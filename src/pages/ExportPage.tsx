...
import type {
  ExportOptions as ElectronExportOptions,
  ExportProgress,
...
interface NianbanUploadConfig {
  baseUrl: string
  deviceId: string
  dyadId: number
}

interface TaskNianbanUploadState {
...
interface ExportTaskPayload {
  ...
  nianbanUpload?: NianbanUploadConfig
}
...
interface ExportTask {
  ...
  nianbanUpload?: TaskNianbanUploadState
}
...
function ExportPage() {
  ...
  const [uploadToNianban, setUploadToNianban] = useState(false)
  const [nianbanBaseUrl, setNianbanBaseUrl] = useState('')
  const [nianbanDeviceId, setNianbanDeviceId] = useState('')
  const [nianbanDyadId, setNianbanDyadId] = useState('')
    ...
    try {
      if (next.payload.scope === 'sns') {
        ...
        if (!result.success) {
          ...
          const resultSessionOutputPaths = (
            result.sessionOutputPaths &&
            typeof result.sessionOutputPaths === 'object'
              ? result.sessionOutputPaths
              : {}
          ) as Record<string, string>
          const nianbanUploadConfig = next.payload.nianbanUpload
          const targetSessionId = next.payload.sessionIds[0] || ''
          const targetJsonPath = targetSessionId
            ? String(resultSessionOutputPaths[targetSessionId] || '').trim()
            : ''
          ...
          if (nianbanUploadConfig && targetSessionId && targetJsonPath) {
            void (async () => {
              try {
                ...
                updateTask(next.id, task => {
                  if (!task.nianbanUpload) return task
                  return {
                    ...task,
                    nianbanUpload: uploadResult.ok
                      ? {
                          ...task.nianbanUpload,
                          status: 'success',
  ...
  const enqueueExportTask = useCallback((title: string, payload: ExportTaskPayload): string => {
    const task: ExportTask = {
      ...
      performance: payload.scope === 'content' && payload.contentType === 'text'
        ...
        : undefined,
      nianbanUpload: payload.nianbanUpload
        ? {
            status: 'idle',
            dyadId: payload.nianbanUpload.dyadId
          }
        : undefined
    }
  ...
  const createTask = async () => {
    ...
    const snsOptions = exportDialog.scope === 'sns'
      ...
      : undefined
    if (shouldUploadToNianban && nianbanValidationErrors.length > 0) {
      window.alert(nianbanValidationErrors[0])
      return
    }
    const nianbanUpload = shouldUploadToNianban && resolvedNianbanDyadId != null
      ? {
          baseUrl: nianbanBaseUrl.trim(),
          deviceId: nianbanDeviceId.trim(),
          dyadId: resolvedNianbanDyadId
        }
      : undefined
    const title =
  ...
  const activeSingleSessionId = exportDialog.scope === 'single' ? exportDialog.sessionIds[0] : ''
  const activeSingleSession = activeSingleSessionId
    ? sessions.find(session => session.username === activeSingleSessionId)
    : undefined
  ...
  const nianbanValidationErrors = useMemo(() => {
  ...
  ) && nianbanValidationErrors.length === 0
        ...
        <div className="export-dialog-overlay" onClick={closeExportDialog}>
          <div className="export-dialog" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="dialog-body">
              ...
              {shouldShowNianbanUploadSection && (
                <div className="dialog-section">
                  <div className="dialog-switch-row">
                    ...
                    <button
                      type="button"
                      className={`dialog-switch ${uploadToNianban ? 'on' : ''}`}
                      aria-pressed={uploadToNianban}
                      aria-label="切换导出后上传至念伴"
                      onClick={() => {
                        setUploadToNianban(prev => {
                          const next = !prev
                          if (next) {
                            setOptions(current => ({ ...current, format: 'json' }))
                          }
                          return next
                        })
                      }}
                  ...
                  {uploadToNianban && (
                    <div className="nianban-upload-fields">
                      <label className="dialog-field-stack">
                        <span>念伴后端地址</span>
                        <input
                          type="text"
                          className={`dialog-text-input ${nianbanBaseUrl.trim() ? '' : 'invalid'}`}
                          value={nianbanBaseUrl}
                          placeholder="https://your-nianban-api.example.com"
                          onChange={event => setNianbanBaseUrl(event.target.value)}
                        />
                      </label>
                      <label className="dialog-field-stack">
                        <span>X-Device-Id</span>
                        <input
                          type="text"
                          className={`dialog-text-input ${nianbanDeviceId.trim() ? '' : 'invalid'}`}
                          value={nianbanDeviceId}
                          placeholder="device-id"
                          onChange={event => setNianbanDeviceId(event.target.value)}
                        />
                      </label>
                      <label className="dialog-field-stack">
                        <span>目标 Dyad ID</span>
                        <input
                          type="text"
                          inputMode="numeric"
                          className={`dialog-text-input ${resolvedNianbanDyadId == null ? 'invalid' : ''}`}
                          value={nianbanDyadId}
                          placeholder="123"
                          onChange={event => setNianbanDyadId(event.target.value)}
                        />
                      </label>
                      {nianbanValidationErrors.length > 0 && (
                        <div className="dialog-error-list">
                          {nianbanValidationErrors.map(error => (
